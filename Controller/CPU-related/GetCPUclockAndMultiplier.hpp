...
namespace CPU
{
    ...
    float & r_fVoltageInVolt)
  {
    ...
    DEBUGN( FULL_FUNC_NAME << " numMinimumSteps:" << numMinimumSteps
    ...
    mask = (DWORD) SetThreadAffinityMask(GetCurrentThread(), affMask);
      ...
      QueryPerformanceCounter(&lPerfCounterValueBeforeStart);
      lTimeStampCounterBefore.QuadPart = //ReadTSCinOrder(/*&StartL,&StartH*/ affMask);
        ReadTSCinOrderFromEnglishWikipedia();
      QueryPerformanceCounter(&lStartPerfCounterValue);
  ...
  //      unsigned waitTimeInMs = 1;
      ...
      //    ){
        ...
        do
        {
          QueryPerformanceCounter(&lPerfCounterValueBeforeEnd);
          //rdtsc(&EndL,&EndH);
          lTimeStampCounterAfter.QuadPart = //ReadTSCinOrder(affMask);
            ReadTSCinOrderFromEnglishWikipedia();
          QueryPerformanceCounter(&lEndPerfCounterValue);
...
#ifdef _DEBUG
        DEBUGN( FULL_FUNC_NAME << " # loop passes: " << numLoopPasses);
#endif
//          secondsBetweenTSCstartAndEnd = (double)(timerStepsPassed.QuadPart) /
//            (double)(lPerfCounterStepsPerSecond.QuadPart);

        DEBUGN( FULL_FUNC_NAME << " end TSC QPC: sec:" << secondsPassed
...
//          error = 1.0f/(float)timerStepsPassed.QuadPart;
  ...
  //          ;
          ...
          DEBUGN( FULL_FUNC_NAME << " TSC diff:" << numTSCticksElapsed.QuadPart
        ...
        {
          DEBUGN( FULL_FUNC_NAME << " secondsPassed >= " << MIN_TIME_SPAN_IN_S)
          clock = - 2.0;
        }
      }
      else
