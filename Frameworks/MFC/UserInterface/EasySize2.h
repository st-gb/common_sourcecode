...
// "\" wird benutzt, um Zeilemumbrüche zu ermöglichen für
//eine höhere Lesbarkeit,
//denn #define-Anweisungen dürften sonst nur 1 Zeile haben
#ifndef __EASYSIZE_H_
#define __EASYSIZE_H_
//Konstanten(Ersetzungen)
#define ES_BORDER 0xffffffff
#define ES_KEEPSIZE 0xfffffffe
#define ES_HCENTER 0x00000001
#define ES_VCENTER 0x00000002
//Konstanten, Ende
//Funktions-Deklarationen,Anfang
#define DECLARE_EASYSIZE \
void __ES__RepositionControls(BOOL bInit);\
void __ES__CalcBottomRight(CWnd *pThis, BOOL bBottom,\
int &bottomright, int &topleft, UINT id, UINT br,\
int es_br, CRect &rect, int clientbottomright);
//Funktions-Deklarationen,Ende
#define INIT_EASYSIZE __ES__RepositionControls(TRUE);\
...
//Funktions-DEFINITIONEN,Anfang
void class::__ES__CalcBottomRight(CWnd *pThis, BOOL bBottom, \
	int &bottomright, int &topleft, UINT id, UINT br, int es_br, \
	CRect &rect, int clientbottomright) {\
if(br==ES_BORDER) bottomright = clientbottomright-es_br;\
else if(br==ES_KEEPSIZE) bottomright = topleft+es_br;\
else { CRect rect2;\
//GetWindowRect:screen coordinates of the upper-left and lower-right corners.
pThis->GetDlgItem(br)->GetWindowRect(rect2);\
//ScreenToClient:The new coordinates are relative to the upper-left corner of the CWnd client area.
 pThis->ScreenToClient(rect2);\
bottomright = (bBottom?rect2.top:rect2.left) - es_br;}}\
void class::__ES__RepositionControls(BOOL bInit) \
{ CRect rect,rect2,client; GetClientRect(client);
