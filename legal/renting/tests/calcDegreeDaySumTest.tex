...
\usepackage[ngerman]{babel}%%für deutsche Anführungszeichen
...
%%Outputs the parameter text in the colour that signals success.
\newcommand\TUBlnThSiTenNiFiHighlightSuccess[1]
{
  \color{green}#1\color{black}
}

%%Outputs the parameter text in the colour that signals error.
\newcommand\TUBlnThSiTenNiFiHighlightError[1]
{
  \color{red}#1\color{black}
}

%%Use this for each highlighting, so the highlight style (for example the color)
%% can be exchanged at this central point.
\newcommand\TUBlnThSiTenNiFiHighlightText[1]
{
...
%  \TUBlnThSiTenNiFiCalcHeatngDegDayPromilleFullYears
 ...
 \end{itemize}
  ...
  {%%"then" branch
    \TUBlnThSiTenNiFiHighlightSuccess{Number of days matches expected number.}
  }%%"then" branch
  {
...
\newcommand\testFirstOctTwoThsdTwntyFveTillThrtySepTwoThsdTwntySx{
 \TUBlnThSiTenNiFiCalcHeatngDegDaySumTest
  {2025}{10}{01}{2026}{09}{30}{
   1000}{365}
}

\newcommand\outputGregCalAndGerHeatngDegPrmllTbl
{
  ...
  %\foreachitem\GregorianMonthDaysCnt\in\TUBlnThSiTenNiFiGregorianMonthDaysCnt⟩{
  % \GregorianMonthDaysCountcnt{}: \GregorianMonthDaysCount\par }
...
%\color{orange}
Use „debug“ option for „fp“ package if „fp“ calculation errors exist to see the
cause.
\newline

All days are included, so the 01.May-02.May count as 2 days, 01.May-01.May as
 only 1 day.

\outputGregCalAndGerHeatngDegPrmllTbl
%
...
%  92.0}{23}
 ...
 \testFirstOctTwoThsdTwntyFveTillThrtySepTwoThsdTwntySx
